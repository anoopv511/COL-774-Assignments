...
cnn =Sequential()
cnn.add(Conv2D(64, kernel_size=(3, 3),activation="linear",strides=2,padding='same',input_shape=(28,28,1)))
...
cnn.add(Conv2D(64, kernel_size=(3, 3),activation="linear",padding='same'))
cnn.add(LeakyReLU(alpha=0.1))
cnn.add(Conv2D(32, kernel_size=(3, 3),activation="linear",padding='same'))
cnn.add(LeakyReLU(alpha=0.1))
cnn.add(MaxPooling2D(pool_size=(2, 2)))
cnn.add(Dropout(0.2))
cnn.add(Flatten())
...
for i in range(5):
	cnn.fit_generator(datagen.flow(x_train,y_train_categorical,batch_size=128),steps_per_epoch=5000,epochs=5,verbose=1)

	name = 'cnn_aug1_' + str(5*(i+1))
	print(name)
